...
def calculate_performance_metrics(strategy_returns, returns_df):
    ...
    annual_risk_free_rate = returns_df['acc_rate'].mean() * TRADING_DAYS_PER_YEAR
    sharpe_ratio = (annualized_return - annual_risk_free_rate) / annualized_volatility if annualized_volatility > 0 else 0

    # 2. Calculate Alpha and Information Ratio against benchmarks
    y = strategy_returns
    benchmark_cols = ['low_risk', 'momentum', 'quality', 'size', 'value', 'spx']
...
def plot_performance_summary(strategy_returns, returns_df, metrics):
    ...
    # 3. Create and print the summary table
    ...
    metrics_table['Value'] = metrics_table.apply(
        lambda row: f"{row['Value']:.2%}" if "Return" in row['Metric'] or "Alpha" in row['Metric'] or "Vol" in row['Metric']
                    else f"{row['Value']:.2f}",
        axis=1
    )
